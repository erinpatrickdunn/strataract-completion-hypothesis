...
- $S_{\text{EC}}$: Einstein–Cartan gravity (tetrad $e^a_{\phantom{a}\mu}$ + spin connection $\omega^{ab}_{\phantom{ab}\mu}$)
- $S_{\text{geo}}$: Dirac-type spinor with quartic self-interaction ($\lambda > 0$)

**VARIATION:**

$$\frac{\delta}{\delta e^a_{\phantom{a}\mu}} \;\Rightarrow\; G_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa\left(T_{\mu\nu} + \alpha C_{\mu\nu}\right) + \text{Term}(3)$$

$$\frac{\delta}{\delta\omega^{ab}_{\phantom{ab}\mu}} \;\Rightarrow\; T_{\lambda\mu\nu} = \frac{\kappa\alpha}{2}\,\varepsilon_{\lambda\mu\nu\rho}\,A^\rho$$
...
The total action on spacetime manifold M, in terms of the tetrad $e^a_{\phantom{a}\mu}$ and spin connection $\omega^{ab}_{\phantom{ab}\mu}$, is:
...
Variation with respect to the tetrad $e^a_{\phantom{a}\mu}$ yields:

$$G_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa\left[T_{\mu\nu}^{\text{matter}} + \alpha C_{\mu\nu} + T_{\mu\nu}^{\text{torsion}}\right]$$

where $C_{\mu\nu} = \rho\,\eta\,u_\mu u_\nu$ is the leading-order geometric state tensor and $T_{\mu\nu}^{\text{torsion}} \sim A_\mu A_\nu - \tfrac{1}{2}A_\rho A^\rho g_{\mu\nu}$.

Variation with respect to the spin connection $\omega^{ab}_{\phantom{ab}\mu}$ gives the Cartan equation:
