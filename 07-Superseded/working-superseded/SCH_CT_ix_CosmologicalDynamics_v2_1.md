...
## Working Derivation Document — v2.1 | June 2026

> **⚠️ v2.1 FREEZE NOTICE — Section P.10.5 (Branch 2) is SUPERSEDED
> PENDING CLEAN-ROOM CANONICALIZATION, one level further than the v2
> "SUPERSEDED IN v2" notice below already indicated.** A clean-room
> re-derivation of the cosmological Dirac equation
> (`SCH_CleanRoom_Rederivation_v1.md`, June 2026) found that the very
> IVN-I corrected system this v2 revision was built on (Appendix P v13,
> Section P.7.7.3/P.7.7.3a) does not itself survive re-derivation. The
> clean-room package finds $\dot\eta=-3H\eta$ exactly in both branches
> — no $\kappa\alpha A^0P$ sourcing at all — which, if it holds under
> independent check, means the v2 caveats below (written on the
> assumption that such sourcing exists and needs quantifying) are
> themselves testing the wrong thing. This is a **freeze-and-annotate
> patch only**: the v2 body text (Sections P.10.5.0a–P.10.5.4, P.10.6,
> P.10.9, P.10.10) is left unrewritten below, exactly as it was in v2,
> per the project-wide decision not to migrate the clean-room result
> piecemeal. The consolidated tracking item is **IVN-CT8-Dirac-1**
> (Appendix P v13.1, Section P.7.7.10), which supersedes IVN-I-3 and
> everything downstream of it in this document. Do not treat the v2
> Branch 2 sections below as reflecting the current best understanding
> — they are historical, retained for the record, pending the
> canonical rebuild that follows once IVN-CT8-Dirac-1 closes. If the
> clean-room result holds, that rebuild is *simpler* than v2's own
> framing suggests: $\eta$ decouples from the chirality sector
> entirely, and the job becomes rebuilding the $(A^0,P)$ oscillator
> alone rather than a coupled three-variable system.
...
*[v2.1: See the freeze notice at the top of this document. Everything*
*from here through the end of P.10.5 is frozen pending IVN-CT8-Dirac-1*
...
- **Eight open questions** and **eight IVN items** from v1 are identified,
  ...
  corrected Branch 2 system. **(v2.1: this bullet describes v2's own status
  and is now itself frozen — see the header notice. IVN-I-3 is no longer the
  operative item; IVN-CT8-Dirac-1, Appendix P v13.1 Section P.7.7.10, is.)**
...
its conclusions can be treated as more than provisional — **(v2.1: and, per
the clean-room package, that coupled three-variable system may not exist at
all; $\eta$ may decouple exactly. Confirming this is IVN-CT8-Dirac-1's job,
not this document's.)** All extensions required additional structure beyond
the closed variational theory of Appendix P v13.1.

---

*SCH CT-ix Working Derivation — v2.1 | June 2026*
*Revised from v2: freeze-and-annotate notice added throughout Section*
*P.10.5, pointing to IVN-CT8-Dirac-1 (Appendix P v13.1) as the*
*consolidated open item, superseding IVN-I-3. No body text of v2 rewritten,*
*per the project-wide decision not to migrate the clean-room finding*
*piecemeal. Revised from v1: Section P.10.5 (Branch 2) corrected per IVN-I;*
*Branch 1 unaffected. See header changelogs for full accounting.*
*Not for citation without author approval.*
*Requires independent verification at all IVN-, IVN-I-, and*
*IVN-CT8-Dirac-1-marked steps.*
