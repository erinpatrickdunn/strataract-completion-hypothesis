...
| C = tangential velocity of S³ surface | 0.3 | **DERIVED** | Photon as zero-spin surface wave. C is a cosmic readout, not a constant. |
| W-pressure is the origin of inertia | 0.3 | **DERIVED** | Resistance to knot reorientation in rising slice-attachment pressure. |
| C asymptote = escape velocity from S³ slice | 0.3 | **DERIVED** | W-pressure rises without bound as knot approaches tangential velocity. |
...
| Q_μν unique at quadratic order in ψ | 2.1, 2.5 | **THEOREM** | Fierz completeness + three filters. Regime-conditional. Appendix P, Theorem 1. |
| η = ψ̄ψ is w-spin magnitude | 2.3 | **THEOREM** | Lorentz scalar bilinear = geometric measure of knot w-rotation. Appendix P, Theorem 2. |
| GR exact recovery when A_μ = 0 | 2.1, 6.1 | **THEOREM** | W-spin → 0 limit. Torsion vanishes algebraically. Appendix P, Step 16. |
| Spin quantization from S³ closure | 0.4 | **DERIVED** | S³ ≅ SU(2). Closure condition gives integer and half-integer naturally. |
...
| Light predominates at epoch boundaries | 0.6 | **DERIVED** | W-pressure → 0 as c(t) → 0. Matter dissolves to light phase. |
| Bounce = matter-light-matter phase transition | 0.6 | **DERIVED** | No singularity possible — matter phase ends before turnaround. |
| S³ topology compatible with spinor field | 2.9 | **COMPATIBILITY** | SU(2) group manifold = S³. Appendix P, P.7.6. |
| BH bounce condensate frequency f_cond ~ M⁻² | 2.10 | **DIM. ESTIMATE** | From Theorem 4 diffusion timescale. Appendix P, P.7.5. |
...
| Angular diameter turnaround on S³ | 6.8 | **PREDICTION** | Follows from S³ geometry. Single free parameter R_universe. |
...
The strataract condensate is a propagating field — it travels with the stellar matter and diffuses beyond it on timescale τ_diff ~ R²m_eff/ℏ (Theorem 4, Appendix P). The lensing follows the condensate. The gas has no condensate to contribute. The offset is geometric stripping, not dark matter.

## 1.4 Supporting Patterns

**JWST early massive galaxies.** JWST has identified galaxies at z ~ 10-16 that are anomalously massive, anomalously compact, and anomalously early relative to ΛCDM predictions. The bounce cosmology of Section 0.6 provides the explanation: each prior cycle deposits more net baryonic matter into the current cycle's initial conditions than a single-origin Big Bang would produce. Galaxies forming early in the current cycle did so in a matter-richer environment than ΛCDM assumes. Full treatment in Appendix C.
...
The phenomenological form η(Z,N) = |∫ψ_nuclear(r) Y₀₀(θ,φ) dV|² f(β₂, β₄, ...) remains valid as a computational tool for nuclear calculations. The spinor bilinear is the underlying physical definition.
...
The suppression function f(τ) = τ_coll/(τ_coll + τ_coh) approaches 1 when the collision timescale greatly exceeds the coherence recovery timescale and approaches 0 in the reverse limit.

## 2.4a The η Evolution Equation

**u^μ∇_μη = -Γ_decoh η + Γ_recoh(1 - η)**

Both rates are derived from S_geo via the finite-temperature effective potential (Appendix P, Theorem 3):

Γ_decoh = (α/m²)(λρ)²κ(T)
Γ_recoh = (α/m²)(λρ)²κ(T)f(T)
...
The observational proxy at galactic scales is λ_R = ⟨V⟩/√(⟨V²⟩ + ⟨σ²⟩) — the projected stellar spin parameter from IFU kinematics. This measures the degree to which stellar orbits are coherently rotating rather than randomly dispersed, which is the galactic-scale expression of w-spin coherence.
...
Four ΛCDM mechanisms are considered and found insufficient to produce the predicted monotonic λ_R-lensing dependence at fixed stellar mass:
...
General Relativity is recovered exactly when A_μ = 0 — the isotropic ground state of the spinor field, corresponding to zero net w-spin. In this limit, torsion vanishes algebraically, C_μν = 0, and the field equation reduces to:
...
The geometric state spinor ψ is defined on a curved spacetime manifold with covering group SU(2) × SU(2). The group manifold of SU(2) is, as a smooth manifold, the three-sphere S³. SU(2) and S³ are the same object — not a coincidence of notation but a mathematical identity. A spinor field whose symmetry group is SU(2) × SU(2) is most naturally and consistently defined on a spatial topology that is itself S³.

This was stated in Section 0.6 in physical terms: the cosmos is an S³ rotating on the w-axis. The mathematical statement is: S³ is the unique compact topology on which the spin structure canonically determined by the SU(2) × SU(2) covering group is already present without additional input. Full argument in Appendix P, Section P.7.6.
...
On S³ of radius R_universe, the angular diameter distance is:
