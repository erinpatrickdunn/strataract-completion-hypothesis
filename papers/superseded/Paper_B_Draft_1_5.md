...
Revised from Draft 1.4: Targeted extension incorporating the galactic engine physical picture (*SCH_GalacticEngine_PhysicalPicture_v1*). Section 1 contrast class paragraph tightened to reflect that condensate field coherence — driven by black hole population organization — is the relevant variable, not only stellar orbital coherence. Section 7.1 (NANOGrav) updated to note that the antipodal convergence mechanism gives individual black hole condensate hum a cosmological destination and couples it to global $S^3$ modes. Section 7.2 (Antipodal CMB) updated to note that the antipodal correlation receives an active contribution from condensate wave convergence, in addition to the passive topological mode structure. New Section 7.4 introduces the rotation curve flattening radius versus central black hole mass correlation as a PROPOSED TEST falsifiable against existing data. Section 8.2 updated with CT-xix and CT-xx in the priority ordering. Section 9 Conclusion updated accordingly. All content from Draft 1.4 is carried forward unchanged. Supersedes Draft 1.4.

Prior revision summary: Draft 1.4 replaced Section 2 in full with the beta\_z replication. Draft 1.3 corrected the JWST framing and added the antipodal CMB correlation search. Draft 1.2 added the JWST standard ruler topology test, post-merger lensing timescale test, Ampère force anomaly, and NANOGrav exploratory direction.
...
## **7.4 Rotation Curve Flattening Radius versus Central Black Hole Mass \[PROPOSED TEST\]**
...
This test requires no new observations and no new data collection. The SPARC sample and existing dynamical BH mass compilations provide the required inputs today. It is the lowest-cost new proposed test in the programme after the antipodal CMB correlation search of Section 7.2, and unlike that search it does not depend on any pending theoretical calculation for its falsification condition — the null prediction (no correlation at fixed stellar mass) is sharp and model-independent. The test should be run immediately.

The quantitative prediction of the slope $\beta$ requires CT-vii (black hole condensate propagator). Running the correlation test before CT-vii is complete is appropriate: a null result provides an immediate falsification of the mechanism at no theoretical cost; a positive result motivates prioritizing CT-vii to produce the quantitative slope prediction for comparison.
...
The recommended priority ordering for the experimental programme is: (1) MaNGA-DES weak lensing cross-match (Paper A Test 4.1) -- uses existing public data, directly tests the primary galactic-scale prediction; (2) SLACS metallicity-lensing test (Paper A Test 4.2) -- uses existing public data, tests the nuclear-scale prediction; (3) **Rotation curve flattening radius versus central black hole mass correlation (Section 7.4)** -- uses existing public data, no new observations required, sharp null prediction, no theoretical prerequisites; (4) JWST standard ruler topology test (Section 4) -- uses existing public data for z ≤ 8, single free parameter, clean falsification condition; (5) Bi-209 laboratory calibration (Paper A Section 5) -- pins alpha and m\_eff, unlocks quantitative predictions for Sections 5, 6, and 7; (6) Resistive/superconducting calorimetric comparison (Section 6) -- after Bi-209 provides the expected signal magnitude.
...
All outcomes of further testing are scientifically valuable. The beta\_z signal reported in Section 2 is the prediction on record before the DES Y6 Metadetection weak lensing shape catalogue is released. The lensing staircase is independent of every modelling choice made in Section 2. A null result in the lensing cross-match at sufficient sensitivity would constitute the strongest available falsification of the galactic-scale SCH prediction. A null result in the rotation curve flattening correlation at fixed stellar mass would falsify the coherence-forcing mechanism as the origin of halo extent. A positive antipodal correlation in the Planck data would be multi-signal evidence for $S^3$ topology. All directions sharpen the framework's empirical profile. The DES Y6 lensing cross-match remains the decisive near-term test; the rotation curve flattening correlation is the new test that can be run today.
