...
| Gap 7 — Chirality inversion across bounce and sympathetic nucleation | **FROZEN — PENDING IVN-CT8-Dirac-1** | The v13 entry (IVN-I's convention-corrected monodromy calculation) is **superseded pending clean-room canonicalization**, not operative. A clean-room re-derivation (`SCH_CleanRoom_Rederivation_v1.md`, June 2026) found $\eta$'s dilution law is structurally protected ($\dot\eta=-3H\eta$ exactly, no $\kappa\alpha A^0P$ sourcing) and that IVN-I's own $(A^0,P)$ system does not survive re-derivation either. The qualitative conclusion that chirality inversion per cycle is non-generic is *not* contradicted by this finding and may survive canonicalization, but the specific bilinear system in P.7.7.3/P.7.7.3a below should not be cited as current. See Section P.7.7.10 (IVN-CT8-Dirac-1) for the consolidated open item, and `SCH_IVNI_MonodromyCorrection_v1.md` for its own updated supersession notice. |
...
| Gap 16 — Cosmological dynamics: solution structure of the modified Friedmann system | **CLOSED for Branch 1; Branch 2 FROZEN pending IVN-CT8-Dirac-1** | CT-ix (June 2026): Branch 1 fully characterised — two-phase dynamics (stiff-condensate $a \propto t^{1/3}$, then dust-condensate); analytic solutions in each phase; explicit $a_{\text{max}} = \kappa m\eta_0/3$ and $R_{\text{universe}}$ as function of $m\eta_0$; CMB quadrupole constraint translates to $m\eta_0 \geq 9c^4/(8\pi G)$. **Confirmed unaffected by the v13.1 freeze.** Branch 2's late-time-attractor claim (v13's caveat, itself based on IVN-I's now-superseded system) is frozen pending the same clean-room canonicalization as Gap 7 — see Section P.7.7.10. If the clean-room package's structural result holds, Branch 2 is *simpler* than v13 assumed ($\eta$ decouples from the chirality sector entirely), but the $(A^0,P)$ oscillator itself still needs to be rebuilt from the canonical system once IVN-CT8-Dirac-1 closes. See SCH_CT_ix_CosmologicalDynamics_v2.1.md. |
...
| **IVN-CT8-Dirac-1b** | Independently re-derive the contorsion-sourced axial coupling coefficient ($\tfrac{\kappa\alpha}2A^0\gamma^0\gamma^5$) from the raw $K^{ab}_c$ contraction, matching the P.9.4.1-style spatial integration on $S^3$ but with the full $\omega=\overset\circ\omega+K$ connection. The clean-room package imported this coefficient's magnitude from the Cartan-equation normalization and verified only its reality, not its precise value/operator structure from first principles. | CR-3 | HIGH | **PARTIAL** — see `SCH_IVN-CT8-Dirac-1b_TorsionCoefficient_v1.md`. The raw operator structure ($\gamma^0\gamma^5$) and magnitude are confirmed by direct index contraction. But $A^0$ is not external — it is sourced by the same $\psi$, so the coupling must be treated via auxiliary-field elimination (substitute the algebraic torsion solution into the action, then vary), which turns it into a genuine quartic self-interaction $\propto(A^0)^2$. Varying that term correctly reproduces a real $\dot\eta$ contribution but an **inconsistent, unremoved imaginary residual in $\dot P$**. Not closed — opens IVN-CT8-Dirac-1b-i. |
| **IVN-CT8-Dirac-1b-i** | Identify and fix whatever is missing in the self-consistent (auxiliary-field-elimination) treatment of the torsion-sourced quartic term such that all four bilinear equations ($\dot\eta,\dot J^0,\dot P,\dot A^0$) come out simultaneously real. Until this closes, whether $\eta$ is exactly protected in Branch 2 is **reopened**, not confirmed — Branch 1 ($A^0=0$) is unaffected. | — (new) | **CRITICAL** | **OPEN** — blocking. Should be pursued alongside, not after, 1c. |
...
**1b is now PARTIAL** (see `SCH_IVN-CT8-Dirac-1b_TorsionCoefficient_v1.md`):
the raw operator structure is confirmed, but properly accounting for the
coupling's self-sourced nature (auxiliary-field elimination) produces an
internally inconsistent result, opening **IVN-CT8-Dirac-1b-i (CRITICAL)**.
**Updated order: 1a (closed) → {1b-i, 1c in parallel} → 1d.** 1b-i and 1c
should be pursued together rather than sequentially, since both bear on
whether the Branch 2 bilinear system is even internally consistent. Only
after IVN-CT8-Dirac-1 fully closes (1b-i and 1c independently resolved)
should the canonical rewrite of P.9.4.2, P.9.5.3, P.7.7.3, P.7.7.3a, and
CT-ix Section P.10.5 be performed, in one pass. The freeze established in
this section remains in effect for the Dirac/bilinear sector until that
rewrite — **if anything, 1b's finding strengthens the case for the
freeze**, since it shows the question of whether $\eta$ is protected in
Branch 2 is genuinely reopened, not merely pending confirmation. 1a's
closure
