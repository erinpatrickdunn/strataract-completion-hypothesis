...
Revised from Draft 2.5: **Audit-integration pass (implementation phase).** This revision integrates five approved corrections arising from independent audit and meta-audit review, none of which changes an equation, a coefficient, or a numerical claim. **(AA-1)** The citation to `SCH_Project_Wide_Audit_Report_v1.md` in the Draft 2.5 revision note below is corrected: that document could not be located in the repository, and the citation is corrected to accurately describe its status rather than assert inspection of an unconfirmed source; a shared editorial note is added immediately below that note. **(AA-3)** The epistemic status table's rows for "Spin quantization from S³ closure," "Uncertainty principle is geometric," "Orbital shapes from w-axis excursions," "W-pressure is the origin of inertia," and "$c$ asymptote = escape velocity" are decomposed or reclassified, and six claims previously asserted only in §§0.3–0.5's prose without a table entry — the relativistic-effects analogy list, the Pauli exclusion principle, the spin-statistics theorem, the discrete mass spectrum, the double slit, and quantum entanglement — receive new table rows and classification notes, applying the same (a)/(b) decomposition standard Draft 2.5 already applied to two other rows. **(AA-4)** Sections 2.2, 2.3, and 2.8 are revised to distinguish the dimensionful bilinear $\eta=\bar\psi\psi$ from the normalized, dimensionless coupling-efficiency use of $\eta$ in Section 2.3, and to correct Section 2.8's claim that $A_\mu=0$ alone implies $C_{\mu\nu}=0$ — these are independent conditions on two distinct bilinears, both required for GR recovery. **(AA-5)** Section 2.1's citation "Full derivation in Appendix P, Theorems 1–4" is replaced with an accurate account of what each cited theorem establishes, and an explicit note that the tetrad variation itself is not written out anywhere in the current corpus. Supersedes Draft 2.5.

Revised from Draft 2.4: **Audit-integration pass.** This revision applies, in place and in full text (no patch-list shortcuts, per the standard established in the companion Appendix P v17 audit-integration pass), the six Paper A action items identified during an independent audit review [see editorial note below], corresponding to what that review's own internal numbering designated Section 3 (items A-1 through A-6). Per the Reconstruction Firewall stated in that review, nothing here changes an equation, a coefficient, or a numerical claim. Every change is one of: **RECLASSIFY** (a label corrected to match its true epistemic status), **DECOMPOSE** (a bundled claim split into sub-claims with separate labels), or **UPDATE** (a stale cross-document version citation corrected to match the current state of the corpus). Specifically: §2.4a's "no free parameters remain" claim for $\Gamma_{\text{decoh}},\Gamma_{\text{recoh}}$ is tagged as downstream-dependent on Appendix P's own (now-corrected) Gap 3 status (A-1); the epistemic table's "Mass = w-axis spin magnitude" and "$c$ = tangential velocity" rows are each decomposed into an established mathematical/dimensional piece and a conditional physical-narrative piece, mirroring Appendix P's Theorem 0 and Theorem 5 decompositions (A-2, A-3); §6.9.0's "Two-tax redshift" claim is decomposed into its established additive structure and its still-open quantitative evaluation (A-4); every live reference to the rotation-curve-flattening prediction via $\lambda_{\text{cond}}\sim M^2$ is retagged `SUPERSEDED`, matching Appendix P v17's explicit suspension of that scaling pending CT-vii(c) recomputation (A-5); and all companion-document version pointers are synced from Appendix P v14 to Appendix P v17 (A-6). Section 2.10a's chirality-sector language and the Theorem 1/$Q_{\mu\nu}$ citations in Sections 2.1 and 2.5 were confirmed sound on audit and are carried forward with no content change beyond the version-pointer sync. Supersedes Draft 2.4.

**[Editorial note, added Draft 2.6: the audit-integration items described in the paragraph above were originally attributed to `SCH_Project_Wide_Audit_Report_v1.md`. That file could not be located in the repository as of this correction. Each item's substance has been independently checked against the current canonical corpus and confirmed sound on its own terms — see the document-level audit reports for the verification record. Only the citation is corrected here; no classification, equation, or numerical claim listed above is altered by this note.]**
...
*Note added Draft 2.5 (audit item A-2, A-3): two rows below — "Mass = w-axis spin magnitude" and "$c$ = tangential velocity of $S^3$ surface" — are decomposed in place following the same split applied to Appendix P v17's Theorem 0 and Theorem 5. Each row is split into an (a) sub-row carrying the established mathematical/dimensional content and a (b) sub-row carrying the physical-narrative content, which is a Category-3 premise per Appendix P's CP-02 and its Theorem-5 analogue.*

*Note added Draft 2.6 (audit item AA-3, implementation pass): the rows below for "Spin quantization from S³ closure," "Uncertainty principle is geometric," "Orbital shapes from w-axis excursions," "W-pressure is the origin of inertia," and "$c$ asymptote = escape velocity" are decomposed or reclassified this revision, and new rows are added for claims previously asserted only in §§0.3–0.5's prose without a table entry — the relativistic-effects analogy list, the Pauli exclusion principle, the spin-statistics theorem, the discrete mass spectrum, the double slit, and quantum entanglement — applying the same (a)/(b) decomposition standard used above.*
...
| (2a) Mass = w-axis spin magnitude — the labeling of $\eta=\bar\psi\psi$ as "w-spin magnitude" | 0.2 | **CONDITIONAL — this is the same Category-3 premise as Appendix P's CP-02, not an independent derivation from soliton geometry.** | The scalar-bilinear mathematics ($\eta=\bar\psi\psi$ is a Lorentz scalar) is established (Appendix P Theorem 2); the identification of that scalar with a literal w-axis rotational departure is an elaboration of Axiom 0.2 itself, not a derivation of it. Falls out of soliton geometry *given* the axiom; does not independently establish the axiom. |
| (2b) Zero w-spin = zero mass = light, as a formal consequence of (2a) | 0.2 | **DERIVED, conditional on (2a)** | Given the identification in (2a), the zero-mass limit follows algebraically (Appendix P Theorem 0, Step 4; Theorem 6). |
| (3a) $c$ = tangential velocity of $S^3$ surface, given the stated setup | 0.3 | **DERIVED** | $c(t)=\omega(t)R_{\text{cosmic}}(t)$ follows from the definition of $c$ as maximum signal speed realized by a tangential-motion limit on a rotating $S^3$ (Appendix P Theorem 5, sub-claim 11a). $c$ is a cosmic readout, not a constant, given this setup. |
| (3b) Photon as minimum-w-spin ($\eta \approx \eta_{\min} > 0$) surface wave, and the specific inference from lensing to $\eta_{\min}$ | 0.3 | **CONDITIONAL** | Lensing establishes $\eta_{\text{photon}}\neq0$; it does not by itself establish the specific minimum-w-spin-surface-wave narrative used to explain it (Appendix P Theorem 5, sub-claim 11b — same family as CP-02). |
| Length contraction, clock slowing, relativistic mass increase, twin paradox, relativity of simultaneity — geometric reinterpretation of SR | 0.3 | **OPEN** (new row, Draft 2.6, audit item AA-3) | Narrative analogies only; none derives the Lorentz factor $\gamma=1/\sqrt{1-v^2/c^2}$ or any other quantitative SR relation from the w-tilt geometry. Grouped as one row since all five share the identical gap. |
| W-pressure is the origin of inertia | 0.3 | **OPEN** (reclassified, Draft 2.6, audit item AA-3) | No functional form for how w-pressure rises with velocity is shown, nor a demonstration that it reproduces relativistic momentum/energy's specific quantitative form; narrative only. |
| $c$ asymptote = escape velocity from $S^3$ slice | 0.3 | **OPEN** (reclassified, Draft 2.6, audit item AA-3) | Same gap as the row above — no quantitative demonstration that this reproduces $E=\gamma mc^2\to\infty$ as $v\to c$, or any specific functional form; narrative only. |
...
| $Q_{\mu\nu}$ unique at quadratic order in $\psi$ | 2.1, 2.5 | **THEOREM** | Fierz completeness + three filters. Regime-conditional. Appendix P, Theorem 1. Confirmed sound on audit; no change. |
| $\eta = \bar{\psi}\psi$ is w-spin magnitude | 2.3 | **DECOMPOSED, matching (2a)/(2b) above.** Scalar-bilinear mathematics: **THEOREM**. Physical w-spin identification: **CONDITIONAL (CP-02 analogue).** | Lorentz scalar bilinear = geometric measure of soliton w-rotation, the latter an elaboration of Axiom 0.2. Appendix P, Theorem 2 (math) / Theorem 0 (narrative). |
| GR exact recovery when $A_\mu = 0$ | 2.1, 6.1 | **THEOREM** | W-spin $\to$ 0 limit. Torsion vanishes algebraically. Appendix P, Step 16. |
| (4a) $S^3\cong SU(2)$, the double cover of $SO(3)$, admits both integer and half-integer closure solutions for any quantity subject to an $SU(2)$-valued closure condition | 0.4 | **ESTABLISHED** (decomposed, Draft 2.6, audit item AA-3) | Standard group theory, appropriately referenced — the same mathematical fact underlying ordinary spin-1/2 quantization, not an SCH-specific result. $SU(2)$ double-covers $SO(3)$. |
| (4b) A physical soliton threading $S^3$ is, in fact, subject to such a closure condition, and this is what produces observed particle spin quantization | 0.4 | **OPEN** (decomposed, Draft 2.6, audit item AA-3) | No dynamical argument is shown for why a soliton's internal state must satisfy this closure condition, nor what the tracked "phase" physically is. Asserted by analogy ("the same mathematics that quantizes electron orbitals, generalized to S³ geometry"), not derived. |
| Pauli exclusion principle as topological exclusion | 0.4 | **OPEN** (new row, Draft 2.6, audit item AA-3) | No mechanism is shown connecting "two solitons cannot occupy the same w-rotational state" to the actual mathematical content of the Pauli principle (antisymmetrization of the multi-particle wavefunction under exchange). Asserted, not derived. |
| Spin-statistics theorem as a consequence of closure type | 0.4 | **OPEN**, doubly conditional on (4b) (new row, Draft 2.6, audit item AA-3) | Depends on (4b) (itself now OPEN) plus an unaddressed further step connecting "commute/anti-commute under closure type" to the actual spin-statistics theorem's field-theoretic content. |
| Discrete mass spectrum from quantized w-spin | 0.4 | **CONDITIONAL**, doubly dependent on (2a) and (4b) (new row, Draft 2.6, audit item AA-3) | Requires both the w-spin-as-mass identification (2a, already `CONDITIONAL`) and spin quantization's physical instantiation (4b, now `OPEN`) to hold. |
| Uncertainty principle is geometric | 0.5 | **OPEN** (reclassified, Draft 2.6, audit item AA-3) | That a 4D object's 3D cross-section is a region rather than a point is a generic geometric fact, not SCH-specific. Reaching the actual Heisenberg relation ($\Delta x\Delta p\geq\hbar/2$, involving conjugate momentum-space variables) from this is not shown. |
| Orbital shapes from w-axis excursions | 0.5 | **OPEN** (reclassified, Draft 2.6, audit item AA-3) | No derivation of the actual angular/radial dependence (spherical harmonics, Coulomb radial functions) that gives s/p/d/f orbitals their specific mathematical form; narrative analogy only. |
| Double-slit interference as a soliton-coupling event | 0.5 | **OPEN** (new row, Draft 2.6, audit item AA-3) | Narrative correspondence only; no derivation of the actual interference pattern's mathematical form or the measurement/decoherence formalism it would need to reproduce. |
| Quantum entanglement via shared 4D topology | 0.5 | **OPEN** (new row, Draft 2.6, audit item AA-3) | Narrative correspondence only; no mechanism shown for reproducing the specific (Bell-inequality-violating) statistical correlations quantum entanglement exhibits. |
...
| $S^3$ topology compatible with spinor field | 2.9 | **COMPATIBILITY** | $\mathrm{SU}(2)$ group manifold = $S^3$. Appendix P, P.7.6. (Note, Draft 2.5: Appendix P v17 relabels its own Gap 6 to match this row exactly, resolving a prior cross-document mismatch — CI-01 — in this row's favor.) |
| BH bounce condensate frequency $f_{\text{cond}} \sim M^{-2}$ | 2.10 | **DIM. ESTIMATE — SUPERSEDED (Draft 2.5, audit item A-5)** | From Theorem 4 diffusion timescale, Appendix P, P.7.5. Appendix P v17 (§P.7.5.2, §P.11.5) retracts the formula this scaling was computed from; the $M^{-2}$ exponent itself is not currently established, pending CT-vii(c). See the note appended to Section 2.10 below. |
| Chirality inversion per cosmological cycle ($A^\mu \to -A^\mu$) | 2.10a | **ESTABLISHED — CONFIRMED NON-GENERIC; MAGNITUDE PENDING BI-209** | Appendix P v17 (June 2026; carrying forward v14's closure unchanged) closes the derivation: three independent routes (direct-bilinear; a twice-attempted, self-correcting 4D-first route) converge on the torsion self-coupling coefficient. Chirality inversion is confirmed non-generic — it requires a non-topologically-protected fine-tuning of action parameters, not a default consequence of $S^3$ geometry. The monodromy phase and within-cycle matter surplus are now fully well-posed numerically, pending only the Bi-209 calibration. Confirmed sound on audit; no content change. See Section 2.10a. |
...
| (6.9.0a) Two-tax redshift, additive structure: $z_{\text{obs}}=z_{\text{expansion}}+z_{\text{drag}}$ | 6.9.0 | **DERIVED** | Follows from Theorem 5's established (3a) sub-claim plus the existence of a nonzero, if unquantified, photon-condensate coupling. |
| (6.9.0b) Quantitative evaluation of $z_{\text{drag}}$ and the separating signature's detectability | 6.9.0 | **OPEN, inherits Appendix P Gap 8 (audit item A-4)** | Requires $\sigma(\omega)$ from CT-xiii, currently an open calculational target. Not yet a derived quantitative prediction. |
...
| Rotation curve flattening radius correlates with central BH mass | 2.11 | **SUPERSEDED (Draft 2.5, audit item A-5)** | The specific mechanism as stated derives the flattening radius from $\lambda_{\text{cond}}=c/f_{\text{cond}}\sim M^2$; Appendix P v17 retracts the propagation-frequency scaling this rests on (§P.7.5.2, §P.11.5), pending CT-vii(c) recomputation. Whether *some* correlation between flattening radius and central BH mass survives under a corrected scaling law is not established either way by this retraction — it is simply not currently derivable at the stated functional form. See the note appended to Section 2.11 below. Data-gap finding of Paper B Section 7.4 is unaffected by this change (it concerns sample availability, not the theoretical scaling). |
...
*(Draft 2.5 note, audit item A-2: this identification is the Axiom stated here, and is elaborated formally — not independently derived — in Appendix P's Theorem 0. See the epistemic table row (2a)/(2b) above.)*
...
*(Draft 2.5 note, audit item A-3: "light bends, therefore photons couple, therefore their w-spin is nonzero" is sound — this is sub-claim (3b)'s established floor. "It is minimum" — i.e., the specific identification $\eta_{\text{photon}}=\eta_{\min}$ as opposed to some other nonzero value — is the conditional part of (3b); see Appendix P Theorem 5's decomposition.)*
...
*(Draft 2.6 note, audit item AA-3: the five relativistic-effects bullets above are narrative reinterpretations, not derivations — none produces the Lorentz factor or any other quantitative SR relation from the stated geometry. See the epistemic table's new row for this list.)*
...
*(Draft 2.6 note, audit item AA-3: "follows" in the three sentences above is narrative, not derived — see the epistemic table's (4b) row and the two new rows for Pauli exclusion and the discrete mass spectrum. The closure-condition mathematics itself (integer/half-integer solutions from $SU(2)$'s double cover, sub-claim 4a) is standard and established; its physical instantiation by an actual soliton, and everything built on top of it in this paragraph, is not.)*
...
*(Draft 2.6 note, audit item AA-3: all four results in this section — the uncertainty principle, orbital shapes, the double slit, and entanglement — are geometric analogies, not derivations. None produces a quantitative match to the corresponding standard-quantum-mechanical result. See the epistemic table's corresponding rows, all reclassified `OPEN` this revision.)*
...
*(Draft 2.5 note: the diffusion timescale $\tau_{\text{diff}}$ quoted here is Appendix P's original Theorem 4 formula, itself retracted in Appendix P v15/v17 (superseded by Theorem 4b's ballistic/diffusive damped-wave transport). This is the same dependency already tracked in Paper B Section 5 for the post-merger lensing test; see that document's own audit item B-1 for the fuller treatment. No change is made to this paragraph's qualitative Geometric Stripping mechanism, which does not depend on the specific retracted timescale formula.)*
...
**NANOGrav gravitational wave background.** The 2023 NANOGrav detection of a stochastic background in the nHz range is conventionally attributed to supermassive black hole binary mergers. The black hole bounce resonance of Section 2.10 provides an alternative contribution: individual black holes driving condensate propagation at frequencies scaling as M⁻², which for intermediate-mass black holes falls in the nHz range. Full treatment in Section 2.10 and Paper B Section 7.1. *(Draft 2.5 note: the specific $M^{-2}$ frequency-band placement inherits the same SUPERSEDED status noted at Section 2.10 below; the qualitative coherence-time discriminator described in Paper B Section 7.1.3 is unaffected.)*
...
This equation is derived from the Einstein-Cartan-Dirac action with a geometric state spinor field ψ. The spinor field ψ is the mathematical representation of the soliton structure described in Section 0.2. The action S_geo is the formal encoding of the soliton's w-axis dynamics. Stated as the result of tetrad variation of $S_{\text{geo}}$ (Appendix P, §P.1.3); the variation itself is asserted there, not carried out step-by-step in the current corpus. Theorems 1–4 establish related but distinct results used elsewhere in this paper — $Q_{\mu\nu}$'s uniqueness (Theorem 1, §2.4), $u^\mu$ normalization and $\eta$'s scalar status (Theorem 2, §2.3), the mass-generation potential (Theorem 3), and transport rates (Theorems 4a/4b) — none of them is the metric-variation derivation itself. **[Documentation gap, identified Draft 2.6 (audit item AA-5): the explicit tetrad variation of $S_{\text{geo}}$ (§P.1.1's action) producing the field equation of §P.1.3 is not written out anywhere in the current corpus. This is a genuine, previously unnamed gap — its resolution is a derivation task for Appendix P, not attempted here.]**
...
*(Confirmed sound on audit — no content change to this section beyond the citation correction above and the version-pointer sync applied throughout this document.)*
...
The isotropic ground state is the state of zero net w-spin, defined as the joint condition $\eta=0$ **and** $A_\mu=0$ (where $A_\mu=\bar\psi\gamma_\mu\gamma^5\psi$ is the axial current) — two independent bilinears of the same field, both required to vanish; see the classification note in §2.3 and the correction in §2.8. This is the symmetric 4D sphere configuration of Section 0.5 — the lowest energy soliton configuration, the configuration to which matter naturally tends. Departure from this state is what sources C_μν.
...
*(Draft 2.5 note, audit item A-2: as elsewhere in this document, "$\eta = \bar\psi\psi$ is a Lorentz scalar" is established mathematics; "$\eta$ *is* w-spin magnitude" is the physical identification carried from Axiom 0.2, elaborated rather than independently proven in Appendix P's Theorem 0. Both statements are used together throughout this section, as they are throughout the framework; the distinction matters for epistemic bookkeeping, not for the physics used here.)*
...
**[CLASSIFICATION note, added Draft 2.6 (audit item AA-4).** $\eta\equiv\bar\psi\psi$ as it appears in Theorem 0's mass term and in $C_{\mu\nu}=\rho\eta u_\mu u_\nu$ (§2.5) is dimensionful — $[\text{mass}]^3$ in natural units, since $\bar\psi\psi$ is built from two spinor fields each of dimension $[\text{mass}]^{3/2}$. The "$\eta=0$ to $\eta=1$" language above cannot refer to this same dimensionful quantity directly; it refers to an implicitly normalized, dimensionless version, denoted here $\hat\eta\equiv\eta/\eta_{\max}$ for some normalization scale $\eta_{\max}$. **This normalization scale is not derived or specified anywhere in the current corpus.** Where this document (and Papers B and C) write "$\eta$" in a $[0,1]$, qualitative-efficiency sense, "$\hat\eta$" is meant; where "$\eta$" appears inside a dimensioned equation (the mass term, $C_{\mu\nu}$), the dimensionful $\bar\psi\psi$ is meant. This distinction was previously left implicit throughout. Determining $\eta_{\max}$'s value and physical origin is a new, currently unaddressed item — it is not attempted here.]**
...
**[CLASSIFICATION, added Draft 2.5 (audit item A-1).** This claim is `CONDITIONAL — DOWNSTREAM DEPENDENCY` on Appendix P's Theorem 3 (Gap 3), which the companion Appendix P v17 audit-integration pass downgrades from CLOSED to `CONDITIONAL — PROVENANCE UNDOCUMENTED`: no Matsubara sum, self-energy diagram, or regularization scheme is exhibited anywhere in Appendix P to derive these functional forms from $S_{\text{geo}}$ — they are asserted, and their qualitative shape is physically reasonable, but the phrase "no free parameters remain" should be read as "no *additional* free parameters beyond $\{\alpha,\lambda,m\}$ are introduced by this ansatz," not as "this has been derived from first principles." This does not change how the rates are used elsewhere in this paper — the functional forms are retained as working hypotheses — but the strength of the claim in this paragraph is corrected.]**

In the limit T → 0: Γ_decoh → 0, η → 1, maximum geometric coupling. In the limit T >> T_c: η → 0, C_μν → 0, exact GR recovered. The critical temperature T_c ~ m_eff/k_B is the temperature above which the cosmos is too hot to sustain w-spin coherence — the condensate melts.
...
*(Confirmed sound on audit — Theorem 1's citation here is clean and unembellished; no content change.)*
...
General Relativity is recovered exactly when the spinor field is in the isotropic ground state, defined (§2.2) as the joint condition $\eta=0$ **and** $A_\mu=0$. These are logically independent conditions on two distinct bilinears of the same field ($\eta$ a scalar, $A_\mu$ an axial vector) — nothing in the corpus derives one from the other. $A_\mu=0$ is, by itself, sufficient for torsion to vanish algebraically (Cartan equation, Appendix P §P.1.3), giving $T_{\mu\nu}^{\text{torsion}}=0$. $\eta=0$ is, separately and independently, what is required for $C_{\mu\nu}=\rho\eta u_\mu u_\nu=0$. The isotropic ground state is the physical configuration in which both hold simultaneously — this joint vanishing is part of the ground state's definition (§2.2), not a one-implies-the-other derivation. With both conditions satisfied, the field equation reduces to:
...
This was stated in Section 0.6 in physical terms: the cosmos is an $S^3$ rotating on the w-axis. The mathematical statement is: $S^3$ is the unique compact topology on which the spin structure canonically determined by the $\mathrm{SU}(2)\times\mathrm{SU}(2)$ covering group is already present without additional input — a compatibility/minimal-input result, not a proof that no other topology could accommodate that covering group under some other construction. Full argument in Appendix P, Section P.7.6.

*(Note, Draft 2.5: this section's own hedged language — "compatibility" — was already correctly conservative prior to this revision; the audit's item A-5 does not touch this section directly, but Appendix P v17's corresponding item, audit item P-3, brings Appendix P's own STATUS SUMMARY table into line with this section's label, resolving a cross-document mismatch that previously existed only on the Appendix P side.)*
...
The quadratic torsion term — Term 3 — dominates at Planck-scale densities. Black hole interiors reaching Planck density are firmly in the Term 3 regime. The spin-spin repulsion grows as ρ², reversing the collapse when the Planck threshold is reached. The collapsed matter is gravitationally bound and cannot escape the event horizon — it re-collapses, bounces again, and repeats continuously.
...
**[CLASSIFICATION, added Draft 2.5 (audit item A-5).** The $M^{-2}$ scaling above is computed from the diffusion-timescale formula $\tau_{\text{diff}}\sim R_s^2 m_{\text{eff}}/\hbar$ (Appendix P's original Theorem 4). Appendix P v17 retracts this formula outright for the vacuum/dilute regime the Schwarzschild-radius calculation sits in (Section P.11.3), and explicitly voids the entire numerical table computed from it (Section P.7.5.2) as well as the mass-scaling exponent itself, pending recomputation as CT-vii(c). This is `SUPERSEDED`, not merely uncertain: the specific $M^{-2}$ law and every number derived from it in this document (the frequency-band table entries referenced downstream in Paper B Sections 7.1 and 7.4, and this paper's own Section 2.11 rotation-curve-flattening claim below) should be read as suspended pending that recomputation, not as an established dimensional estimate awaiting only calibration. The *quality factor* $Q$ and the qualitative claim that black holes are near-perfect condensate resonators are separate claims, resting on Appendix P's Theorem 4a (carrier structure) and P.7.5.3, and are unaffected by this retraction.]**
...
**What changed and what didn't, across the full revision history.** The physical mechanism (sympathetic nucleation) and its connection to the JWST early-massive-galaxy anomaly (Section 1.4) are unchanged since Draft 2.0. What Draft 2.2 retracted was the earlier framing that cycle-to-cycle alternation was close to automatic. What Draft 2.3 added was the recognition that $\eta$ and the chirality sector are coupled in Branch 2, flagged at the time as requiring independent verification. What Draft 2.4 added was that the verification is complete: the coupling mechanism and its coefficient are established by three independent routes, closing the freeze that had been in place on this sector of Appendix P. Draft 2.5 adds only a version-pointer sync and the (16c) cross-reference noted above; no physical claim in this section changes. Both the magnitude of the within-cycle surplus and the cycle-to-cycle chirality pattern remain parametric — not because the calculation is unsettled, but because they depend on the numerical value of $\alpha$ and $m$, which only the Bi-209 calibration can supply. See Appendix P Section P.7.7 for the full derivation and Section P.7.7.10 for the complete resolution record.

## 2.11 The Galactic Engine: Antipodal Coupling and Coherence-Forced Halos

*The physical picture developed here is stated first in plain language. The formal derivation requirements are CT-xix and CT-xx in Appendix P. Epistemic status: PREDICTION for the antipodal coupling mechanism; CONJECTURE for the coherence-forced halo picture and the M-sigma consequence; SUPERSEDED (Draft 2.5, audit item A-5) for the rotation curve flattening correlation as specifically derived below from $\lambda_{\text{cond}}\sim M^2$.*
...
**[CLASSIFICATION, added Draft 2.5 (audit item A-5).** The paragraph above is the source of this document's rotation-curve-flattening prediction, and it rests entirely on $\lambda_{\text{cond}}=c/f_{\text{cond}}\sim M^2$, which in turn rests on the same retracted diffusion-timescale formula flagged at Section 2.10 above. This prediction is `SUPERSEDED`: not falsified, but currently without a derived functional form to test, pending Appendix P's CT-vii(c) recomputation of the propagation-frequency scaling. The *qualitative* picture — that a transition radius exists somewhere between structured near-field interference and a unified far-field hum, and that this transition plausibly sets the flattening radius — is not itself retracted; only the specific $M^2$ scaling used to make it a numerical, falsifiable prediction is. Paper B Section 7.4's data-gap finding (fewer than 10 galaxies in the SPARC/direct-BH-mass cross-match) is a separate, unaffected issue — that finding is about data availability, not about this scaling law, and would apply regardless of which scaling law is eventually confirmed.]**

**The M-sigma consequence.** The M-sigma relation — the empirically tight correlation between central black hole mass and galaxy velocity dispersion — has been known for decades but lacks a first-principles explanation. In the galactic engine picture it is a consequence of the coherence forcing scale. The SMBH sets the coherence forcing range for the entire galaxy through $f_{\text{cond}} \sim M^{-2}$: a more massive SMBH forces coherence to larger radii, maintaining a larger halo, producing higher anomalous orbital velocities throughout the galaxy, and therefore higher velocity dispersion. The M-sigma relation is the observational signature of this scaling. A quantitative derivation requires CT-xx (thermodynamic consistency of the coherence-forcing mechanism), which establishes the range at which a given SMBH can maintain halo coherence as a function of its mass and emission power. *(Draft 2.5 note: this paragraph's own $f_{\text{cond}}\sim M^{-2}$ dependence inherits the same SUPERSEDED tag noted above; the qualitative M-sigma story — larger SMBH, larger forcing range, higher dispersion — does not require the specific exponent, but the exponent as stated is not currently derivable pending CT-vii(c).)*
...
(1) No statistically significant correlation between rotation curve flattening radius and central black hole mass, in a sample with reliable measurements of both, falsifies the coherence-forcing mechanism as the origin of halo extent. *(Draft 2.5 note: as currently stated, this falsification condition presumes the $M^2$ functional form that is itself suspended per the classification box above. A null correlation would still be informative, but a positive correlation with a different exponent than the (currently un-derivable) predicted one would not, by itself, falsify the qualitative coherence-forcing mechanism — only the specific scaling. This condition should be read as provisional pending CT-vii(c).)*
...
**[CLASSIFICATION, added Draft 2.5 (audit item A-5).** Item (5) in the list above inherits the same `SUPERSEDED` status as Section 2.11's rotation-curve-flattening prediction and Section 2.10's frequency scaling: the specific functional form is currently suspended pending Appendix P's CT-vii(c). Items (1)–(4) are unaffected.]**
...
|---|---|
| Lagrangian architecture | CLOSED, conditional on Appendix P's R1–R4 requirement set (CP-01). S_geo is the Einstein-Cartan-Dirac action. Metric variation gives the field equation exactly. |
| Tensor emergence (Q_μν) | CLOSED. Theorem 1: unique rank-2 symmetric divergence-free spinor bilinear tensor at quadratic order. |
| η scalar nature | CLOSED (scalar-bilinear mathematics). The physical "w-spin" identification is CONDITIONAL — see the epistemic table decomposition above (audit item A-2). |
| GR recovery | CLOSED. A_μ = 0 and η = 0 jointly (zero net w-spin) make torsion and $C_{\mu\nu}$ vanish respectively, recovering exact GR — see Section 2.8. |
| Geometric Resonance | CLOSED. Follows from vacuum structure of SU(2) × SU(2) covering group. |
| Γ_decoh, Γ_recoh | **CONDITIONAL — PROVENANCE UNDOCUMENTED** (downgraded, Draft 2.5, audit item A-1). Functional forms stated; explicit Matsubara evaluation not exhibited in Appendix P. |
| Chirality inversion mechanism and coefficient | CLOSED (since Draft 2.4; carried forward unchanged through Appendix P v17). Appendix P v17: three independent derivation routes converge on the torsion self-coupling coefficient. Numerical evaluation of the monodromy phase and matter surplus remains open, pending Bi-209. |
...
- CT-iii: Lensing diffusion timescale measurement — *(Draft 2.5 note: per Appendix P v17, this now targets Theorem 4b's derived ballistic/diffusive transport law rather than the retired Theorem 4 diffusion formula; see Appendix P Section P.11 and CT-vii(b).)*
...
- CT-vii: Black hole condensate propagator — full perturbative mode analysis, now expanded into sub-targets CT-vii(a)–(c) in Appendix P v17, covering the GMOR mass ratio, Landau-damping rates, and the recomputation of the black hole frequency table this document's Sections 2.10 and 2.11 depend on (audit item A-5)
- CT-viii: FLRW reduction and modified Friedmann equations — **closed**
- CT-ix: Derivation of R_universe from action parameters — **closed for Branch 1; Branch 2 mechanism established, magnitude pending Bi-209 (Appendix P v17). Appendix P v17 additionally flags the Branch 2 late-time "parametrically small" claim as its own conditional sub-item (16c), not previously distinguished from the mechanism closure itself.**
- CT-x: Bogoliubov analysis of sympathetic nucleation (PT-2). **Status note (carried from Draft 2.4, unaffected):** this calculation proceeds from the fully-established $(\eta, A^0, P)$ system of Appendix P Section P.7.7.3, superseding every prior provisional system cited in Drafts 2.2 and 2.3. Its input $\langle A^0 \rangle$ is understood to be the accumulated holonomy value discussed in Section 2.10a, not a value fixed by an assumed per-cycle sign inversion.
...
*Conjectures 6.9.1 and 6.9.2 carry CONJECTURE epistemic status. The two-tax redshift framework of Section 6.9.0 is decomposed, as of Draft 2.5, into an established additive-structure sub-claim and an open quantitative sub-claim — see the classification box below and the epistemic table entry (audit item A-4). Proof target CT-xiii.*
...
**[CLASSIFICATION, added Draft 2.5 (audit item A-4).** The additive decomposition $z_{\text{obs}}=z_{\text{expansion}}+z_{\text{drag}}$ is `ESTABLISHED`: given Theorem 5's established (3a) sub-claim for $z_{\text{expansion}}$'s mechanism, and given only that *some* nonzero photon-condensate coupling exists (itself established by lensing, independent of the specific $\eta_{\min}$ narrative flagged conditional elsewhere), the two contributions to redshift are structurally additive and independent to leading order — this is a statement about the structure of the energy-loss bookkeeping, not a claim about the size of either term. The *quantitative evaluation* of $z_{\text{drag}}$ — its actual magnitude, the specific detectability of the "separating signature" described below, and the numerical size of $\sigma(\omega)$ it depends on — is `OPEN`, inheriting Appendix P's Gap 8 status (CT-xiii is an open calculational target with no exhibited photon-condensate vertex derivation, one-loop cross-section calculation, or CMB-monopole consistency check). This document's own epistemic table already listed the surrounding conjectures 6.9.1/6.9.2 as CONJECTURE; this box brings the parent claim of Section 6.9.0 itself into consistent alignment, since it had previously been listed as unqualified DERIVED.]**
...
Draft 2.0 established the correct logical order: the physical picture first, the mathematics as its encoding. The ground floor is three axioms — time is flat, space is flat, matter is a soliton with w-axis spin. From these three axioms, without additional postulates, the following fall out (subject, for the entries flagged, to the decompositions and reclassifications applied throughout this revision history):

The origin of inertia. The origin of mass (subject to the (2a)/(2b) split, audit item A-2). The c asymptote as escape velocity from the S³ slice (subject to the (3a)/(3b) split, audit item A-3; the escape-velocity narrative itself reclassified `OPEN`, audit item AA-3). Length contraction. Clock slowing (both reclassified `OPEN`, audit item AA-3, along with relativistic mass increase, the twin paradox, and relativity of simultaneity). The matter-light phase distinction. Annihilation as w-spin cancellation. Spin quantization from S³ closure (subject to the (4a)/(4b) split, audit item AA-3). The Pauli exclusion principle (reclassified `OPEN`, audit item AA-3). The spin-statistics theorem (reclassified `OPEN`, audit item AA-3). The discrete mass spectrum (reclassified `CONDITIONAL`, audit item AA-3). The uncertainty principle as geometric cross-section (reclassified `OPEN`, audit item AA-3). Electron orbital shapes (reclassified `OPEN`, audit item AA-3). The double slit (reclassified `OPEN`, audit item AA-3). Quantum entanglement without superluminal signaling (reclassified `OPEN`, audit item AA-3). Variable c(t) as cosmic rotation readout. Matter stability as a function of cosmic epoch. Light predominance at epoch boundaries. The bounce as a matter-light-matter phase transition without singularity.

Draft 2.1 adds: Black holes as rotational processors rather than sinks. Antipodal convergence on $S^3$ as the coupling mechanism between local black hole emission and global $\omega(t)$. Coherence-forced halos as the physical origin of what appears as dark matter. The rotation curve flattening radius as a prediction correlated with central black hole mass (as of Draft 2.5, `SUPERSEDED` pending CT-vii(c) — audit item A-5). The M-sigma relation as a consequence of the coherence forcing scale.
...
**Draft 2.4 closed that item.** Appendix P v14 reported that three independent derivation routes — resolving a chain of internal errors across the intervening working papers, including one in the derivation used to reach that draft — converge on the same torsion self-coupling coefficient. Chirality inversion per cycle remains confirmed non-generic, as it has been since Draft 2.2. The mechanism coupling $\eta$ to the chirality sector in Branch 2, open since Draft 2.3, is now established.

**Draft 2.5 was an audit-integration pass, not a physics revision.** It applied six classification corrections (audit items A-1 through A-6) identified by an independent claim-inventory audit of the full document corpus, none of which change an equation, a coefficient, or a numerical prediction: $\Gamma_{\text{decoh}}/\Gamma_{\text{recoh}}$'s "no free parameters remain" claim (§2.4a) was tagged as resting on an unexhibited Matsubara derivation in Appendix P; the "mass = w-spin" and "$c$ = tangential velocity" epistemic-table entries were decomposed into their established mathematical cores and their conditional physical-narrative content, mirroring the corresponding decomposition already applied to Appendix P's Theorem 0 and Theorem 5; the two-tax redshift framework (§6.9.0) was decomposed into its established additive structure and its still-open quantitative evaluation; every live reference to the rotation-curve-flattening prediction's specific $M^2$ scaling (§2.10, §2.11, §6.3, and this Conclusion) was retagged `SUPERSEDED`, matching Appendix P v17's explicit retraction of the propagation-frequency formula that scaling was computed from; and all companion-document version pointers were synced to Appendix P v17.

**Draft 2.6 is a second audit-integration pass, applying four further corrections identified by independent review of Draft 2.5 and confirmed by cross-checking against Appendix P v17 directly.** The citation to the audit report underlying Draft 2.5's own revision note is corrected, since that report could not be located in the repository (audit item AA-1). The epistemic table's treatment of Section 0's remaining narrative claims — spin quantization, the Pauli exclusion principle, the spin-statistics theorem, the discrete mass spectrum, the uncertainty principle, electron orbital shapes, the double slit, quantum entanglement, and the special-relativity analogy list — is brought into line with the standard Draft 2.5 already applied to two other rows (audit item AA-3). Section 2's treatment of $\eta$'s normalization and the joint $\eta=0$/$A_\mu=0$ ground-state condition is clarified, without deriving the missing normalization scale (audit item AA-4). Section 2.1's citation for the modified field equation is corrected to accurately describe what Appendix P's Theorems 1–4 each establish, and to name — without attempting to close — the previously unflagged gap that the tetrad variation itself is not written out anywhere in the current corpus (audit item AA-5). As with Draft 2.5, no equation, coefficient, derivation step, or numerical result is altered anywhere in this document relative to Draft 2.5.

What remains open — deliberately, and stated as such throughout this draft and in Appendix P — is purely numerical for the chirality sector specifically: the monodromy phase, the within-cycle matter surplus, and the size of the Branch 2 correction to the standard $a^{-3}$ dilution law all require the Bi-209 calibration before they become specific numbers rather than calculable forms; the black-hole condensate propagation frequency and the rotation-curve-flattening scaling require CT-vii(c) before they become specific functional forms again at all; and, as of this draft, the normalization scale $\eta_{\max}$ (§2.3) and the tetrad-variation derivation itself (§2.1) are newly named documentation gaps, neither resolved here.

The Einstein-Cartan-Dirac action is the mathematical encoding of this physical picture — not its source, but its formalization, itself conditional on the R1–R4 requirement set flagged in Appendix P (audit item P-7). GR is the exact zero-w-spin limit. The anomaly cluster is a set of predicted consequences, not motivating puzzles.

**The decisive next steps are:** (1) independent expert verification of the four theorems in Appendix P; (2) execution of the MaNGA-DES cross-match test (Test 4.1); (3) execution of the SLACS metallicity-lensing test (Test 4.2); (4) the Bismuth-209 laboratory calibration, now the single remaining gate on every numerical prediction in the chirality sector as well as the nuclear and galactic sectors; (5) the S³ standard ruler test; (6) derivation of the photon-condensate coupling cross section σ(ω) (CT-xiii); (7) derivation of the black hole condensate propagator (CT-vii), which is the prerequisite for both CT-xix (antipodal coupling) and — as of Draft 2.5, more urgently than previously stated, since the current functional form is suspended rather than merely awaiting calibration — the rotation curve flattening prediction's basic functional form; (8) empirical test of the rotation curve flattening radius versus central black hole mass correlation — awaiting both direct dynamical BH mass measurements for a sufficient sample of late-type spirals (data gap confirmed by pipeline execution June 2026; expected from MBHBM* and WISDOM/ALMA programmes) *and*, as of Draft 2.5, a recomputed scaling law from CT-vii(c) to test in the first place; (9) numerical evaluation of the now-established PT-1 monodromy calculation and the Branch 2 late-time correction, both fully well-posed in mechanism and both awaiting only item (4) — with the Branch 2 correction's smallness itself now explicitly flagged as an assertion pending a derived bound, not yet a computed number, independent of item (4); (10) derivation, from $S_{\text{geo}}$'s minimal-coupling structure, of the tetrad variation producing the field equation of Section 2.1, and determination of the normalization scale $\eta_{\max}$ implicit in Section 2.3 — both newly identified documentation gaps as of Draft 2.6.
...
*Appendices A, B, and C are unchanged from Draft 1.5 and are carried forward in the submission package. Appendix P is updated to v17 (Draft 2.5, audit item A-6 — synced from v14).*

---

**End of Paper A — Draft 2.6**

---

*Summary of Draft 2.6 changes relative to Draft 2.5 (full reproduction per the no-elision standard established in the companion Appendix P v17 audit-integration pass): (1) The Draft 2.5 revision note's citation to `SCH_Project_Wide_Audit_Report_v1.md` is corrected to describe an independent audit review whose own source document could not be located, and a shared editorial note is inserted immediately below that note (audit item AA-1). (2) Epistemic table rows for "Spin quantization from S³ closure" decomposed into (4a)/(4b) sub-rows; "Uncertainty principle is geometric," "Orbital shapes from w-axis excursions," "W-pressure is the origin of inertia," and "$c$ asymptote = escape velocity" reclassified from DERIVED to OPEN; six new rows added for the relativistic-effects analogy list, the Pauli exclusion principle, the spin-statistics theorem, the discrete mass spectrum, the double slit, and quantum entanglement (audit item AA-3). (3) Corresponding classification notes inserted in-line after the relevant paragraphs in §0.3, §0.4, and §0.5 (audit item AA-3). (4) The Conclusion's summary paragraph is annotated with parenthetical status flags for each reclassified item, following the same convention already used there for the (2a)/(2b) and (3a)/(3b) splits (audit item AA-3, propagated). (5) §2.2's ground-state definition and §2.8's GR-recovery paragraph are revised to state that $\eta=0$ and $A_\mu=0$ are independent conditions, both required for GR recovery, rather than a single unexamined limit; §2.3 receives a new classification note distinguishing the dimensionful $\eta=\bar\psi\psi$ from a normalized $\hat\eta$, with the normalization scale $\eta_{\max}$ explicitly flagged as undetermined and not derived here (audit item AA-4). (6) §6.5's "GR recovery" row is updated to match the §2.8 correction. (7) §2.1's "Full derivation in Appendix P, Theorems 1-4" citation is replaced with an accurate, itemized account of what each theorem establishes, and a new documentation gap is named regarding the tetrad variation itself (audit item AA-5). (8) The Conclusion's "decisive next steps" list gains a new item (10) reflecting the two documentation gaps named in this revision. (9) No equation, coefficient, derivation step, or numerical result is altered anywhere in this document relative to Draft 2.5.*