...
*Companion paper to: Geometric State as a Gravitational Source Variable: A Multi-Scale Framework and Falsifiable Test Program (Paper A, Draft 2.3). Appendix P v13 contains the formal proofs.*
...
**Prefatory Note -- Draft 1.7**

Draft 1.7 is a version-pointer housekeeping revision only, syncing companion-document references to Paper A Draft 2.3 and Appendix P v13. See the changelog above for what changed upstream and why it does not touch any of Paper B's own analysis.
...
Paper A (Draft 2.3) derives a closed variational framework in which the geometric organizational state of matter constitutes an independent gravitational source variable. The field equation
...
The theoretical priority ordering is: CT-vii (black hole condensate propagator) → CT-viii (FLRW reduction) → {CT-ix, PT-1, CT-xix} in parallel → CT-xx. (PT-1's chirality-inversion calculation was corrected for a metric-convention error in Appendix P v13, IVN-I; the qualitative conclusion — chirality inversion per cycle is not generic — is unchanged, and the sequencing above is unaffected.) CT-xix (antipodal condensate coupling) and CT-xx (thermodynamic consistency of the coherence-forcing mechanism) are new in Appendix P v9 and are the formal prerequisites for converting the galactic engine picture of Paper A Section 2.11 into quantitative predictions. CT-xix is also the prerequisite for the quantitative prediction of the rotation curve flattening slope in Section 7.4.
