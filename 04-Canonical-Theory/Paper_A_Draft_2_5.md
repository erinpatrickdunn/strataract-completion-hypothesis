...
*(Confirmed sound on audit — no content change to this section beyond the version-pointer sync applied throughout this document.)*
...
**Draft 2.5 is an audit-integration pass, not a physics revision.** It applies six classification corrections (audit items A-1 through A-6) identified by an independent claim-inventory audit of the full document corpus, none of which change an equation, a coefficient, or a numerical prediction: $\Gamma_{\text{decoh}}/\Gamma_{\text{recoh}}$'s "no free parameters remain" claim (§2.4a) is now correctly tagged as resting on an unexhibited Matsubara derivation in Appendix P; the "mass = w-spin" and "$c$ = tangential velocity" epistemic-table entries are decomposed into their established mathematical cores and their conditional physical-narrative content, mirroring the corresponding decomposition already applied to Appendix P's Theorem 0 and Theorem 5; the two-tax redshift framework (§6.9.0) is decomposed into its established additive structure and its still-open quantitative evaluation; every live reference to the rotation-curve-flattening prediction's specific $M^2$ scaling (§2.10, §2.11, §6.3, and this Conclusion) is retagged `SUPERSEDED`, matching Appendix P v17's explicit retraction of the propagation-frequency formula that scaling was computed from; and all companion-document version pointers are synced to Appendix P v17. What remains open — deliberately, and stated as such throughout this draft and in Appendix P — is purely numerical for the chirality sector specifically: the monodromy phase, the within-cycle matter surplus, and the size of the Branch 2 correction to the standard $a^{-3}$ dilution law all require the Bi-209 calibration before they become specific numbers rather than calculable forms; and, newly flagged this revision, the black-hole condensate propagation frequency and the rotation-curve-flattening scaling require CT-vii(c) before they become specific functional forms again at all.
...
**End of Paper A — Draft 2.5**

---

*Summary of Draft 2.5 changes relative to Draft 2.4 (full reproduction per the no-elision standard established in the companion Appendix P v17 audit-integration pass): (1) Epistemic table rows for "Mass = w-axis spin magnitude" and "$c$ = tangential velocity of $S^3$ surface" decomposed into (a)/(b) sub-rows (audit items A-2, A-3). (2) "Two-tax redshift" row decomposed into (6.9.0a)/(6.9.0b) sub-rows (audit item A-4). (3) "Rotation curve flattening radius correlates with central BH mass" row retagged SUPERSEDED (audit item A-5). (4) "$S^3$ topology compatible with spinor field" row annotated to note the now-resolved cross-document status match with Appendix P (no status change to this row itself — it was already COMPATIBILITY, the more conservative label, prior to this revision). (5) Classification boxes inserted in-line at: §2.4a (A-1), §0.2/§0.3/§2.3 (A-2, A-3, narrative annotations only, no table duplicated content), §2.10 (A-5), §2.11 (A-5, in three places: the main mechanism paragraph, the M-sigma paragraph, and falsification condition (1)), §6.3 item (5) (A-5), §6.9.0 (A-4). (6) All "Appendix P v14" citations updated to "Appendix P v17" (A-6), with historical revision-note references to v9/v13/v14 in the change-log at the top of this document left untouched as historical record, consistent with the same convention Appendix P v17 itself uses for its own revision history. (7) No equation, coefficient, derivation step, or numerical result is altered anywhere in this document relative to Draft 2.4.*
