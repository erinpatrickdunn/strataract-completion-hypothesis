...
*Companion paper to: Geometric State as a Gravitational Source Variable: A Multi-Scale Framework and Falsifiable Test Program (Paper A, Draft 2.4). Appendix P v14 contains the formal proofs.*
...
**Prefatory Note -- Draft 1.8**

Draft 1.8 is a version-pointer housekeeping revision, syncing companion-document references to Paper A Draft 2.4 and Appendix P v14, the same category of update as Draft 1.7 before it. See the changelog above for what changed upstream — the closure of the chirality/torsion sector previously frozen — and why it does not touch any of Paper B's own analysis. The one substantive update within Paper B itself is confined to Section 8.2's parenthetical note on PT-1, corrected below to reflect that the calculation is now fully specified rather than pending a coefficient resolution.
...
Paper A (Draft 2.4) derives a closed variational framework in which the geometric organizational state of matter constitutes an independent gravitational source variable. The field equation
...
## **7.4 Rotation Curve Flattening Radius versus Central Black Hole Mass \[PROPOSED TEST — AWAITING DATA\]**

*EPISTEMIC STATUS: PROPOSED TEST. New in Draft 1.5. The prediction follows from the galactic engine physical picture of Paper A Section 2.11 and the $f_{\text{cond}} \sim M^{-2}$ scaling of the black hole condensate propagation frequency. The test is falsifiable against existing data with no new observations required. A quantitative prediction requires CT-vii (black hole condensate propagator).*

### **7.4.1 The Prediction**

The condensate hum emitted by a central supermassive black hole of mass $M$ propagates outward at frequency $f_{\text{cond}} \sim \hbar c^4 / (4G^2 m_{\text{eff}} M^2)$, with corresponding wavelength $\lambda_{\text{cond}} = c / f_{\text{cond}} \sim M^2$. This hum maintains rotational coherence in halo matter — keeping the w-spin alignment of orbiting matter above the threshold needed to source $C_{\mu\nu}$ at anomalous levels — within a characteristic radius set by the wave amplitude falloff and the thermal decoherence rate of the halo matter.
...
where $R_{\text{flat}}$ is the rotation curve flattening radius, $M_{\text{BH}}$ is the central black hole mass, and $\beta$ is a power-law index derivable from the condensate propagation physics once CT-vii is complete. The dimensional estimate from $\lambda_{\text{cond}} \sim M^2$ gives $\beta \approx 2$ at leading order, subject to corrections from the halo decoherence profile and the propagation geometry.
...
Step 4. Compare against the null prediction. Standard dark matter models predict no partial correlation between $R_{\text{flat}}$ and $M_{\text{BH}}$ at fixed stellar mass, because the halo structure is determined by total mass, not by the central black hole. SCH predicts a positive partial correlation with slope consistent with $\beta \approx 2$.
...
A realistic estimate is that within 3--5 years, the SPARC cross-match with direct dynamical BH masses will reach 20--40 galaxies, at which point the Section 7.4 test becomes executable with meaningful statistical power.

**Falsification condition (unchanged).** No statistically significant positive partial correlation between $R_{\text{flat}}$ and $M_{\text{BH}}$ at fixed stellar mass, in a sample of $N \geq 30$ galaxies with direct dynamical BH masses and well-resolved rotation curves, falsifies the coherence-forcing mechanism as the origin of halo extent. This condition is well-defined and will be testable when the data exists.
...
The primary observational directions of this paper are not independent. They share the same underlying parameter architecture. The coupling constant alpha and the condensate mass m\_eff appear in all of them. The Bi-209 calibration (Paper A Section 5) is the load-bearing experiment that converts all quantitative predictions from conditional statements to specific numbers. Until it is performed, the predictions of Sections 4, 5, 6, and 7 are parametric: they specify what would be observed for given values of alpha and m\_eff, not the specific numbers to look for.
...
The rotation curve flattening radius versus central black hole mass test (Section 7.4) is correctly specified but cannot currently be executed: pipeline execution in June 2026 confirmed that the cross-match between SPARC and existing direct dynamical BH mass catalogues yields fewer than 10 galaxies, insufficient for a meaningful test. It is removed from the active priority ordering and reclassified as awaiting data. The expected data source is the MBHBM$^*$ Project and WISDOM/ALMA programmes, which are actively measuring direct dynamical BH masses in SPARC-class late-type spirals. The test should be revisited when the cross-match reaches N ≥ 30.

The theoretical priority ordering is: CT-vii (black hole condensate propagator) → CT-viii (FLRW reduction, closed) → {CT-ix, PT-1, CT-xix} in parallel → CT-xx. **Updated in Draft 1.8:** PT-1 (the cosmological chirality-inversion calculation) and the CT-ix Branch 2 dynamics it depends on are, as of Appendix P v14, no longer gated on any open convention or coefficient question — three independently-executed derivation routes converge on the torsion self-coupling coefficient, and the freeze that had been in place on this sector since v13.1 is lifted. Both PT-1 and the CT-ix Branch 2 late-time correction are now gated only on the Bi-209 calibration, the same gate as everything else in this priority ordering. The qualitative conclusion — chirality inversion per cycle is not generic — is unchanged from every prior draft; what has changed is that the calculation behind it is now established rather than provisional. The sequencing above is otherwise unaffected: CT-xix (antipodal condensate coupling) and CT-xx (thermodynamic consistency of the coherence-forcing mechanism) remain open targets, dated to Appendix P v9 for their original specification, unrelated to and unaffected by the chirality-sector closure.

*The exploratory directions of Sections 7.1, 7.2, and 7.3 -- the NANOGrav condensate hum, the antipodal CMB correlation search, and the cosmic void structure -- are not on the critical path but are executable in parallel with the main programme. The antipodal CMB search requires only existing Planck data and standard CMB analysis tools. Section 7.4 is the newest proposed test and is executable immediately with existing data.*
...
This paper has presented one replication result computed from public data (the MaNGA DR17 beta\_z partial correlation, Section 2), one cross-scale consistency check (the Earth flyby anomaly sign-lock and K-cluster, Section 3), four proposed observational and laboratory tests (the JWST standard ruler topology test, the post-merger lensing timescale test, the resistive/superconducting calorimetric comparison, and the rotation curve flattening radius versus central black hole mass correlation, Sections 4–6 and 7.4), and three exploratory observational directions (the NANOGrav condensate hum interpretation, the antipodal CMB correlation search, and the cosmic void structure, Sections 7.1–7.3).

Draft 1.5 added Section 7.4 as a new PROPOSED TEST: the correlation between rotation curve flattening radius and central black hole mass at fixed stellar mass. This test follows from the galactic engine physical picture of Paper A Section 2.11. It requires no new observations, has a sharp null prediction from standard dark matter, and is executable immediately against existing SPARC and dynamical BH mass data. The contrast class in Section 1 is tightened to reflect that condensate field coherence — driven by black hole population organization — is the relevant variable, not only stellar orbital coherence.

Draft 1.8 makes no substantive change to any result in this paper. It updates the theoretical priority ordering of Section 8.2 to reflect that Appendix P v14 has closed the chirality-sector freeze that Draft 1.7 could only note as ongoing. PT-1 is now fully specified and awaits only the same Bi-209 calibration that gates every other quantitative prediction in this paper.

These results and proposals do not confirm the Geometric State Framework. They constitute observational signatures that the framework predicts and that standard models do not currently explain, proposed tests with explicit falsification conditions, and consistency checks across scales. The staircase direction is inconsistent with ΛCDM assembly history predictions and is not driven by the mass estimator. The flyby sign-lock tracks equatorial trajectory geometry rather than spacecraft-specific properties. The JWST mass and timing anomaly has a candidate explanation in the bounce cosmology. The Ampère force anomaly has a candidate mechanism with a discriminating proposed experiment. The antipodal CMB correlation test is executable immediately with existing Planck data. The rotation curve flattening test is executable immediately with existing SPARC and BH mass data. The Bootes void structure is an exploratory direction requiring formal development of the spatial gradient terms of $S_{\text{geo}}$.

All outcomes of further testing are scientifically valuable. The beta\_z signal reported in Section 2 is the prediction on record before the DES Y6 Metadetection weak lensing shape catalogue is released. The lensing staircase is independent of every modelling choice made in Section 2. A null result in the lensing cross-match at sufficient sensitivity would constitute the strongest available falsification of the galactic-scale SCH prediction. A positive antipodal correlation in the Planck data would be multi-signal evidence for $S^3$ topology. All directions sharpen the framework's empirical profile. The DES Y6 lensing cross-match remains the decisive near-term test.

The rotation curve flattening radius versus central black hole mass test (Section 7.4) is correctly specified and its falsification condition is sharp, but it cannot be executed now: the required direct dynamical BH masses for a statistically useful sample of SPARC-type late-type spirals do not yet exist. Pipeline execution in June 2026 confirmed the cross-match yields fewer than 10 galaxies. The test is on record and waiting on data from the MBHBM$^*$ and WISDOM/ALMA programmes.
