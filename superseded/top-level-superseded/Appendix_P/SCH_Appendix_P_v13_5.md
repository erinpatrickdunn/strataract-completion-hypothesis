**Appendix P — v13.5 | June 2026**

Revised from v13.4: **IVN-CT8-Dirac-1b-i CLOSED**
(`SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md`, June 2026), via the
direct-bilinear route from the triangulation strategy specified in
v13.4. Working entirely in explicit spinor components (bypassing the
abstract $\bar\psi\Gamma\psi$/Hermiticity-lemma machinery that had
produced the inconsistency) located the bug: a gamma-algebra error in
Part C's application of the general lemma to the $\Gamma=\gamma^5$
bilinear specifically. The doubled coefficient from auxiliary-field
elimination is independently reconfirmed correct via direct complex-field
variation. The corrected, fully self-consistent Branch 2 system is:
$$\dot\eta=-3H\eta-\tfrac{3\kappa\alpha}2A^0P,\quad \dot J^0=-3HJ^0,\quad
\dot P=-3HP-\left(2m+\left(\lambda-\tfrac{3\kappa\alpha}2\right)\eta\right)A^0,\quad
\dot A^0=-3HA^0+(2m+\lambda\eta)P$$
**Headline finding: $\eta$ IS sourced in Branch 2.** The clean-room
package's central protection claim does not survive; the coupling exists
but with a different mechanism and coefficient than IVN-I originally
proposed. Whether $\eta$ is protected in Branch 2 is no longer an open
question — it is answered, in the negative, pending one remaining
independent check. **The freeze is NOT yet lifted**: per the resolution
document's own recommendation, one further independent re-derivation via
the **4D-first route** (the third triangulation leg, not yet attempted)
should confirm this result before the canonical rewrite of P.9.4.2,
P.9.5.3, P.7.7.3, P.7.7.3a, and CT-ix Section P.10.5 is performed. Version
bumped to v13.5.
...
| Gap 7 — Chirality inversion across bounce and sympathetic nucleation | **FROZEN — RESOLVED PENDING ONE FINAL INDEPENDENT CHECK (4D-first route)** | The v13 entry (IVN-I's convention-corrected monodromy calculation) is superseded. The clean-room package's "no sourcing" claim is also superseded: `SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md` derives, via explicit spinor components (bypassing all abstract bilinear machinery), a fully self-consistent Branch 2 system in which **$\eta$ IS sourced**: $\dot\eta=-3H\eta-\tfrac{3\kappa\alpha}2A^0P$. This is a *third* answer, distinct from both IVN-I's original claim and the clean-room package's claim. All four bilinear equations ($\eta,J^0,P,A^0$) are simultaneously real and internally consistent. The freeze remains in place pending one further independent re-derivation via the 4D-first route (full 4D Einstein-Cartan-Dirac action, Cartan solved algebraically, then reduced to FLRW) before this is promoted from "resolved" to "established." The qualitative PT-1 conclusion (chirality inversion per cycle non-generic) is not directly addressed by this result and should be revisited once the corrected $(\eta,P,A^0)$ system is confirmed. See Section P.7.7.10. |
...
| Gap 16 — Cosmological dynamics: solution structure of the modified Friedmann system | **CLOSED for Branch 1; Branch 2 RESOLVED pending one final independent check** | CT-ix (June 2026): Branch 1 fully characterised — two-phase dynamics (stiff-condensate $a \propto t^{1/3}$, then dust-condensate); analytic solutions in each phase; explicit $a_{\text{max}} = \kappa m\eta_0/3$ and $R_{\text{universe}}$ as function of $m\eta_0$; CMB quadrupole constraint translates to $m\eta_0 \geq 9c^4/(8\pi G)$. **Confirmed unaffected throughout this entire episode; Branch 1 involves no axial current.** Branch 2 is no longer simply "$\eta$ decouples" (clean-room's claim) nor an unresolved inconsistency — it is a genuinely coupled three-variable $(\eta,P,A^0)$ system, with $J^0$ decoupling cleanly and diluting as pure $a^{-3}$. The CT-ix Branch 2 late-time-attractor analysis, bounce-condition scaling, and PT-1 phase estimate all need rebuilding from this corrected system once the pending 4D-first cross-check confirms it. See Section P.7.7.10. See SCH_CT_ix_CosmologicalDynamics_v2.1.md (itself still pending the corresponding rewrite). |
...
**Status update, v13.4 (historical — see v13.5 correction below).** A
dedicated investigation of IVN-CT8-Dirac-1b-i
...
**Triangulation strategy adopted for closing 1b-i (v13.4).** Rather than
...
**Status update, v13.5 — RESOLVED.** The direct-bilinear route (3) was
executed (`SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md`) and found the bug:
a gamma-algebra error in the abstract lemma's application to
$\Gamma=\gamma^5$ specifically — not a conceptual problem with the
antisymmetrization prescription (the leading hypothesis above is now
ruled out) and not a problem with the doubled coefficient (independently
reconfirmed via direct complex-field variation). The corrected system:
$$\dot\eta=-3H\eta-\frac{3\kappa\alpha}2A^0P,\quad \dot J^0=-3HJ^0,\quad
\dot P=-3HP-\left(2m+\left(\lambda-\frac{3\kappa\alpha}2\right)\eta\right)A^0,\quad
\dot A^0=-3HA^0+(2m+\lambda\eta)P$$
is fully real and self-consistent. **$\eta$ is sourced in Branch 2** —
neither IVN-I's original claim nor the clean-room package's "no
sourcing" claim survives; this is a third, independently-derived result.
Per the resolution document's own recommendation, **the freeze is not
yet lifted**: one further independent check via the 4D-first route (1),
not yet attempted, is warranted before this is promoted from "resolved"
to "established" and the canonical rewrite is performed. New tracked
item: **IVN-CT8-Dirac-1b-ii** (4D-first cross-check).
...
| **IVN-CT8-Dirac-1b** | Independently re-derive the contorsion-sourced axial coupling coefficient ($\tfrac{\kappa\alpha}2A^0\gamma^0\gamma^5$) from the raw $K^{ab}_c$ contraction, matching the P.9.4.1-style spatial integration on $S^3$ but with the full $\omega=\overset\circ\omega+K$ connection. The clean-room package imported this coefficient's magnitude from the Cartan-equation normalization and verified only its reality, not its precise value/operator structure from first principles. | CR-3 | HIGH | **RESOLVED (historical: was found internally inconsistent as originally derived)** — see `SCH_IVN-CT8-Dirac-1b_TorsionCoefficient_v1.md` for the original (Part C in error) derivation and `SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md` for the fix. The raw operator structure ($\gamma^0\gamma^5$) and magnitude were confirmed by direct index contraction from the start; Part C's specific $\dot P$ computation contained a gamma-algebra error, now corrected. 1b-i (below) is CLOSED. |
| **IVN-CT8-Dirac-1b-i** | Identify and fix the source of the inconsistency in the self-sourced torsion coupling's derivation, such that all four bilinear equations ($\dot\eta,\dot J^0,\dot P,\dot A^0$) come out simultaneously real. | — (new) | **CRITICAL** | **CLOSED** — see `SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md`. Resolved via the direct-bilinear route (explicit spinor components): the bug was a gamma-algebra error in Part C's abstract-lemma application to $\Gamma=\gamma^5$ specifically, not a conceptual flaw. The doubled coefficient (auxiliary-field elimination) is reconfirmed correct. Corrected system: $\dot\eta=-3H\eta-\tfrac{3\kappa\alpha}2A^0P$, $\dot J^0=-3HJ^0$, $\dot P=-3HP-\left(2m+(\lambda-\tfrac{3\kappa\alpha}2)\eta\right)A^0$, $\dot A^0=-3HA^0+(2m+\lambda\eta)P$ — all real, all self-consistent. **$\eta$ IS sourced in Branch 2**, contra both IVN-I's original claim and the clean-room package's claim. One further independent check (4D-first route) recommended before the freeze lifts. |
...
**1b-i is now CLOSED** (see
`SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md`): the direct-bilinear route
(explicit spinor components, no abstract bilinear machinery) located the
bug — a gamma-algebra error in one specific lemma application, not a
conceptual flaw — and produced a fully self-consistent, all-real Branch 2
system in which $\eta$ **is** sourced. **New item opened:
IVN-CT8-Dirac-1b-ii** — independent re-derivation via the **4D-first
route** (solve the Cartan equation in the full 4D Einstein-Cartan-Dirac
action, substitute to get the 4D quartic fermion action, then reduce to
FLRW), to confirm the resolved system before it is promoted from
"resolved" to "established." **Updated order: 1a (closed) → 1b-i (closed)
→ 1b-ii (current gating item) → 1c**, with 1c now re-scoped to verify the
*torsion-sector* result of 1b-i specifically, not the mass/quartic sector
(which was never in question). The freeze established in this section
**remains in effect** for the Dirac/bilinear sector — 1b-i's resolution
is a strong result, internally cross-checked in three independent ways
within its own derivation, but is not yet the independent, separate
verification the freeze protocol calls for. Once 1b-ii confirms it and
1c completes, the canonical rewrite of P.9.4.2, P.9.5.3, P.7.7.3,
P.7.7.3a, and CT-ix Section P.10.5 should be performed in one pass,
incorporating the corrected three-variable $(\eta,P,A^0)$ Branch 2
system. 1a's
closure
...
---
**End of Appendix P — v13.5**

*June 2026 | Not for citation without author approval*

*Summary of v13.5 changes: IVN-CT8-Dirac-1b-i CLOSED
(`SCH_IVN-CT8-Dirac-1b-i_Resolution_v1.md`) via the direct-bilinear
triangulation route. The bug was a gamma-algebra error in one specific
lemma application, not a conceptual flaw in the framework. The corrected,
fully self-consistent Branch 2 system shows **$\eta$ IS sourced**, with a
new coefficient and mechanism distinct from every prior claim (v12,
IVN-I, and the clean-room package). New item opened: IVN-CT8-Dirac-1b-ii
(4D-first independent cross-check). The freeze remains in effect pending
that check — this result is strong but not yet independently confirmed
in the sense the freeze protocol requires. Branch 1 is confirmed
unaffected throughout. Gap 7 and Gap 16 updated accordingly.*
